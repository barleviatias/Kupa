...
export function hasApprovedTiming(doc) {
  const timing = doc.youtubeTiming;
  return (timing?.status === 'verified'
    || (timing?.status === 'assumed_netflix_zero' && timing.offsetMs === 0)) && Number.isFinite(timing.offsetMs)
    && typeof doc.subtitles?.sourceHash === 'string' && timing.sourceHash === doc.subtitles.sourceHash
...
export function timedMatches(doc, query) {
  ...
  while (matches.length < 30 && attempts++ < 1000) {
    ...
    const enabledTiming = hasApprovedTiming(doc);
    matches.push({ text: cues.map(cue => cue.text).join(' '), cueIds: spans.map(span => span.cueId),
      ...
      playbackStartMs: enabledTiming ? Math.max(0, cues[0].startMs + doc.youtubeTiming.offsetMs) : null });
  }
  return matches;
}
...
export function searchDocuments(documents, query, { enabled = false, allowlist = [] } = {}) {
  ...
  return documents.flatMap(doc => {
    const timed = enabled && (allowlist === null ? hasApprovedTiming(doc) : allowlist.includes(String(doc._id)))
      && doc.subtitleSchemaVersion === 1;
